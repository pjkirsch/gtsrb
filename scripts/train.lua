...
if not opt then
   ...
   cmd:option('-nesterov', false, 'use Nesterov Accelerated Gradient (SGD only)')
...
function train()
   ...
   for t = 1,nTrain,opt.batchSize do
      ...
      local inputs = {}
      local targets = {}
      for i = t,math.min(t+opt.batchSize-1,nTrain) do
         -- load new sample
   ...
   -- time taken
   time = sys.clock() - time
   print("\n==> time to learn 1 sample = " .. (time/nTrain*1000) .. 'ms')
